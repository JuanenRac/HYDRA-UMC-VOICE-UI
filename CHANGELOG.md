...
- **New `tests/test_http_service.py`** (18 tests) - found during a
  ...
  covered "port already in use", not real request/response behavior.
  Real end-to-end tests against a live `VoiceGatewayServer`: token
  required/accepted/rejected, `GET /health` never gated, a real
  `POST /v1/voice/turn` round trip reaching the real intent classifier,
  a malformed body, an invalid `VoiceTurn` shape, an oversized body, and
  - over a real raw socket, since `urllib` always sends one - a genuinely
...
- **New `Dockerfile`**, closing the real gap HYDRA-UMC-COGNITIVE-NODE's
  ...
  yet"). Same `--host`/`--port` CLI the real CM5 systemd unit
  (`systemd/hydra-umc-voice-ui.service`) already runs, bound to
  ...
  Binding beyond loopback still requires a real `HYDRA_UMC_VOICE_UI_TOKEN`
  at run time - `http_service.py`'s own `open_gateway()` already refuses
  otherwise, by design, and this image does not bake one in. Non-root.
