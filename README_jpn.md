...
* **裸の呼び出しが今日も身元/バージョン/役割のみを表示する理由。** それは今も変わらない、軽量な足場チェックのままだ。実際の v0 の作業はそのサブコマンドの背後にある——`analyze-audio`（実際の WAV 読み込み + エネルギー閾値による音声活動検出、`audio.py`）、`parse-intent`（実際のルールベースの意図/エンティティ解析、`intent.py`）、そして `serve`（Watch 向けの実際に認証された音声ターンゲートウェイ、`http_service.py`/`gateway.py`）。これらはいずれも、この README 自身のロードマップが説明する Whisper STT / ニューラル TTS パイプラインではない——それには依然として、この環境にはない本物のモデル依存関係が必要だ。
...
├── Dockerfile                # 認証付き HTTP ゲートウェイ用の最小コンテナイメージ（非 root）
